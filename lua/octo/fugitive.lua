...
function M.clean_fugitive_buffers()
  for _, w in ipairs(api.nvim_list_wins()) do
    print(w, api.nvim_get_current_win(), api.nvim_buf_get_name(api.nvim_win_get_buf(w)))
  ...
  if qf.idx and type(qf.context) == "table" and type(qf.context.items) == "table" then
    ...
    for i=#diff-1, 0, -1 do
      if i then
        ...
        print("la", vim.fn.fnameescape(diff[i+1].filename))
      else
        --vim.cmd(format("rightbelow vert diffsplit %s", vim.fn.fnameescape(diff[i+1].filename)))
        print("rb", vim.fn.fnameescape(diff[i+1].filename))
      end
